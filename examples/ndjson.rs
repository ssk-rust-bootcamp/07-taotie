...
fn deserialize_string_date<'de, D>(deserilizer: D) -> Result<DateTime<Utc>, D::Error>
where
    D: serde::Deserializer<'de>,
{
    let s = String::deserialize(deserilizer)?;

    // format 2019-12-28T05:35:42.771
    let from = s.parse().map_err(serde::de::Error::custom)?;
    let date_time = Utc.from_local_datetime(&from).unwrap();
    Ok(date_time)
}

fn deserialize_string_date_opt<'de, D>(deserilizer: D) -> Result<Option<DateTime<Utc>>, D::Error>
where
    D: serde::Deserializer<'de>,
{
    let s = Option::<String>::deserialize(deserilizer)?;
    match s {
        Some(s) => {
            // format 2019-12-28T05:35:42.771
            let from = s.parse().map_err(serde::de::Error::custom)?;
...
fn main() -> Result<()> {
    let schema = Schema::new(vec![
        ...
        Field::new("last_visited_at", DataType::Date64, false),
        Field::new("last_watched_at", DataType::Date64, false),
        Field::new(
            "recent_watched",
    ...
    let reder = ReaderBuilder::new(Arc::new(schema)).build(reader)?;
    ...
    for batch in reder {
        ...
        let users = serde_json::from_slice::<Vec<User>>(&data)?;
        for user in users {
            println!("{:#?}", user);
        }
    }
